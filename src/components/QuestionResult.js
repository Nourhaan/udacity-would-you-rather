...
class QuestionResult extends Component {
    ...
    render() {
        ...
        const optionOneSelected = this.props.answerQuestions.filter(q => q.id === this.props.question.id &&
            this.props.question.optionOne.votes.includes(localStorage.getItem("logged_user")))[0]

        const optionTwoSelected = this.props.answerQuestions.filter(q => q.id === this.props.question.id &&
            this.props.question.optionTwo.votes.includes(localStorage.getItem("logged_user")))[0]
                ...
                <div className="container result-container">
                    <div className="card-container">
                        <div className="card-header" style={{ backgroundColor: 'grey', color: 'black' }}>
                            <h2>{this.props.users.filter(u => u.id === question.author)[0].name} asks: </h2>
                        </div>
                        <div className="card-content" style={{ backgroundColor: 'black', color: 'white' }}>
                            <div className="info-card" style={{ width: '650px', JustifyContent: 'space-evenly' }}>
                                <img className="card-img-top" src={this.props.users.filter(u => u.id === question.author)[0].avatarURL} alt="Author Avatar"
